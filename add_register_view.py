...
# Inject toggle link in the login view
login_toggle_html = """
                    <div class="text-center pt-2">
                        <button type="button" onclick="toggleAuthView('register')" class="text-xs text-slate-400 hover:text-white transition-colors">
                            Need an account? Create one
                        </button>
                    </div>
"""

# add the register view before the panel view
content = content.replace('            <!-- Admin Portal View (Shown after login) -->', register_html + '\n            <!-- Admin Portal View (Shown after login) -->')

# add the toggle button inside the login form
content = re.sub(r'(<button type="submit" id="admin-login-btn".*?</button>)', r'\1' + login_toggle_html, content, flags=re.DOTALL)
